...
const { poseidon } = require("privacy-pools");
...
describe("Median Optimal test", function () {
    this.timeout(100000000);

    it("should calculate the median of an unsorted array", async () => {
        const circuit = await wasm_tester(path.join(__dirname, "circuits", "median_optimal_test.circom"));

        const unsortedValues = Array.from({length: 69}, () => Math.floor(Math.random() * 77));
        var unsortedValuesHash = 0;
        unsortedValues.map((value) => {
            unsortedValuesHash = poseidon([unsortedValuesHash, value]);
        })
        ...
        const INPUT = {
            "unsortedValuesHash": unsortedValuesHash.toString(),
