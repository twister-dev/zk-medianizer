...
describe("Square sort", function () {
    this.timeout(100000000);

    it("should sort an array", async () => {
        const circuit = await wasm_tester(path.join(__dirname, "circuits", "square_sort_test.circom"));
        ...
        sortedValues.map((value, i) => {
            assert(Fr.eq(Fr.e(witness[i + 1]), Fr.e(value)));
        })
    });
});